...
- (UIImage *)cursorWithFontHeight:(CGFloat)height isTop:(BOOL)top {
    ...
    UIColor *color = NXHColor(28, 107, 222);
...
- (void)setupEvents {
    UISwipeGestureRecognizer* drag = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(userDragGestureDetected:)];
    drag.direction = UISwipeGestureRecognizerDirectionUp|UISwipeGestureRecognizerDirectionDown;
...
-(void)userDragGestureDetected:(UIGestureRecognizer*)regognizer{
...
- (void)fillSelectionAreaInRect:(CGRect)rect {
    UIColor *bgColor =NXHColor(204, 221, 236);// [UIColor redColor];
    //
    CGContextRef context = UIGraphicsGetCurrentContext();
